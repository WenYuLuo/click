...
def random_crop(xs, batch_num, n_total, key):
    ...
    for i in range(0, n_total):
        ...
        while j >= (batch_num * i) and j < (batch_num * (i + 1)):
            ...
            j += 1
        rc_list.append(rc_xs)
    print('sampled from %d clicks' % xs.shape[0])
    np.save('B.npy', rc_xs)
    return rc_list


def random_crop_average_click(xs, batch_num, n_total, key):
    # num = xs.shape[0]
    rc_xs = np.empty((0, 96))

    for i in range(0, n_total):
        average_x = np.zeros((1, 96))
        for j in range(batch_num * i, batch_num * (i + 1)):
            index = j % xs.shape[0]
            temp_x = xs[index]
            # beg_idx = np.random.randint(0, 32)
        ...
        average_x /= batch_num
        rc_xs = np.vstack((rc_xs, average_x))
    # print('sampled from %d clicks' % xs.shape[0])
    np.save('A.npy', rc_xs)
    return rc_xs
...
def train_cnn(n_class, batch_num=20, n_total=500):
    ...
    multi_y = tf.reshape(temp_y, [batch_num, -1])
    # 融合层
    ...
    cross_entropy = -tf.reduce_sum(y_ * tf.log(fuse_out))
    train_step = tf.train.AdamOptimizer(1e-1).minimize(cross_entropy)
    correct_prediction = tf.equal(tf.argmax(fuse_out, 1), tf.argmax(y_, 1))
    ...
    with tf.Session() as sess:
        ...
        for j in range(0, sample_num):
            ...
            out_y = sess.run(fuse_out, feed_dict={x: txs, keep_prob: 1.0})
...
def test_cnn_data_average_click(data_path, label=3, n_class=8, batch_num=20):
    c = label
    npy_files = find_click.list_npy_files(data_path)
    random_index = np.random.permutation(len(npy_files))
    label = np.zeros(n_class)
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(npy_files)):
            ...
            for i in range(len(click_batch)):
                ...
                txs = np.reshape(temp_xs, [1, 96])
                out_y = sess.run(y, feed_dict={x: txs, keep_prob: 1.0})
                ref_y = click_batch[i][1]
                if np.equal(np.argmax(out_y, 1), np.argmax(ref_y)):
                    count += 1
            total_correct += count
            print('correct:', count, 'total:', len(click_batch))
            print('cnn test accuracy (majority voting): ', round(count / len(click_batch), 3))
...
# n_total = 2000
#
train_cnn(n_class=n_class , batch_num=10 , n_total=500)
